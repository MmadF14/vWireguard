...
func New(tmplDir fs.FS, extraData map[string]interface{}, secret [64]byte) *echo.Echo {
	...
	systemStatusString, err := util.StringFromEmbedFile(tmplDir, "system_status.html")
	if err != nil {
		log.Fatal(err)
	}
	...
	templates["system_status.html"] = template.Must(template.New("system_status").Funcs(funcs).Parse(tmplBaseString + systemStatusString))
