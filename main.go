...
var (
    // اطلاعات مربوط به نسخه و بیلد
    appVersion = "development"
    gitCommit  = "N/A"
    gitRef     = "N/A"
    buildTime  = fmt.Sprintf(time.Now().UTC().Format("01-02-2006 15:04:05"))

    // فلگ‌ها و تنظیمات از طریق متغیرهای محیطی یا آرگومان خط فرمان
...
// ------------------------------------------------------
// جاسازی قالب‌ها (templates/)
/*
go:embed templates/*
*/
var embeddedTemplates embed.FS

// جاسازی فایل‌های استاتیک (assets/)
/*
go:embed assets/*
*/
...
func init() {
    // خواندن فلگ‌ها و متغیرهای محیطی
    ...
    util.SessionMaxDuration = int64(flagSessionMaxDuration) * 86_400 // روز به ثانیه
    ...
    // چاپ اطلاعات اپ در لاگ اگر سطح لاگ INFO یا پایین‌تر باشد
    if lvl <= log.INFO {
...
func main() {
    // ایجاد دیتابیس jsondb در پوشه db
    ...
    // راه‌اندازی Goroutine نظارت بر Quota و Expiration
    handler.StartQuotaAndExpirationChecker(db)

    // داده‌های اضافه برای قالب‌ها
    ...
    // جدا کردن پوشه "templates/" از embeddedTemplates
    tmplDir, _ := fs.Sub(fs.FS(embeddedTemplates), "templates")

    // ساخت اولیه کانفیگ WireGuard اگر وجود ندارد
    initServerConfig(db, tmplDir)

    // بررسی و اصلاح SubnetRanges
    ...
    // آماده‌سازی فایل‌های استاتیک از assets
...
// initServerConfig: اگر فایل کانفیگ WireGuard وجود نداشت، یک نمونه جدید می‌سازد
